...
internal class MockNotificationServer
{
    ...
    /// <summary>
    /// Simulates a Dispatch Server, which will send the user to a Notification Server.
    /// </summary>
    /// <returns></returns>
    public async Task ReplyWithXFR()
    {
        ...
        while (true)
        {
            ...
            if (response.Contains("USR") && response.Contains("SSO I"))
            {
                ...
                var echoBytes = Encoding.UTF8.GetBytes($"XFR {TransactionID} NS localhost:1863 U D\r\n");
